# Autopilot Jump Zero Beta v1.1.2-beta.1 - Installer
...
The installer always copies the mod to `<EveJS root>\mods\beta-autopilotJumpZero`,
...
| Native | `StartServer.bat` | `NODE_OPTIONS=<existing> --require "…\mods\beta-autopilotJumpZero\loader.js"` (appended) |
...
`<EveJS root>\_beta-autopilotjumpzero-backup\<timestamp>\` first.
...
| `--force` | Replace an existing `mods\beta-autopilotJumpZero` without archiving it. |
...
```bat
rem --- beta-autopilotJumpZero: preload the server-side loader ---
if exist "%EVEJS_REPO_ROOT%\mods\beta-autopilotJumpZero\loader.js" (
  if defined NODE_OPTIONS (
    set "NODE_OPTIONS=%NODE_OPTIONS% --require="%EVEJS_REPO_ROOT:\=/%/mods/beta-autopilotJumpZero/loader.js""
  ) else (
    set "NODE_OPTIONS=--require="%EVEJS_REPO_ROOT:\=/%/mods/beta-autopilotJumpZero/loader.js""
  )
)
rem --- beta-autopilotJumpZero: end beta-autopilotJumpZero preload ---
```
...
```text
[beta-autopilotJumpZero] v1.1.2-beta.1 active - autopilot warp-in distance 0 m
[beta-autopilotJumpZero] in-memory transform applied: beyonceService autopilot warp-in distance 0 m
```

Inside the container:

```text
docker compose logs server | findstr beta-autopilotJumpZero
```
...
- Native: edit `mods\beta-autopilotJumpZero\.env`. A real environment variable wins
...
`_beta-autopilotjumpzero-backup\`, and leaves the rest of the checkout untouched.
